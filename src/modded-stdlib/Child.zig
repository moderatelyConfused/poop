// Forked from stdlib Child.zig to properly support start_suspended=true on macOS
...
fn spawnPosix(self: *ChildProcess) SpawnError!void {
    ...
    if (pid_result == 0) {
        ...
        // DIFF
        if (native_os == .macos and builtin.link_libc) {
            if (self.start_suspended) {
                posix.kill(std.c.getpid(), posix.SIG.STOP) catch |err| forkChildErrReport(err_pipe[1], err);
            }
        }
        // END DIFF
